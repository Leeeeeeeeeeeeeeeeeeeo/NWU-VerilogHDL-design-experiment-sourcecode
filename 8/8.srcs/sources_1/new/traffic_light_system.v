module traffic_light_system(
    ...
    input rst_n,            // 异步复位（低电平有效，建议接开发板复位键）
    input peak_switch,      // 高峰期模式开关（SW5：拨上=高峰，拨下=正常）
    input [1:0] key_in,     // 按键输入：[1]=BTNU（休眠），[0]=BTNC（应急），低电平有效
...
mode_manager u_mode_manager(
    ...
    .emergency_key(key_state[0]),  // key_state[0]对应应急按键（BTNC）
    .sleep_key(key_state[1]),      // key_state[1]对应休眠按键（BTNU）
    .mode(current_mode),
