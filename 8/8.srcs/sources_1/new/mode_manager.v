...
module mode_manager(
    ...
    input rst_n,                // 低电平有效复位
    input peak_switch,          // 高峰切换开关（SW5：0=正常，1=高峰）
    input emergency_key,        // 消抖后应急按键（BTNC：高=释放，低=按下）
    input sleep_key,            // 消抖后休眠按键（BTNU：高=释放，低=按下）
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        if (peak_edge || 
            ...
            core_rst_n <= 1'b0;  // 复位有效（低电平，持续1个时钟周期）
        end else begin
