module display_controller(
    ...
    // 分频模块生成200Hz扫描时钟（避免数码管闪烁）
    reg [17:0] counter = 0;
    reg clk_200hz = 0;
    
    // 分频参数：100MHz / 200Hz / 2 = 250000（占空比50%）
    ...
    // 根据模式解码显示数字
    reg [3:0] digit_ew_high, digit_ew_low;  // 东西方向：十位、个位
    reg [3:0] digit_ns_high, digit_ns_low;  // 南北方向：十位、个位
    ...
    always @(*) begin
        case (mode)
            ...
            default: begin  // 正常/高峰模式：显示实际倒计时（0-60秒）
    ...
    always @(posedge clk_200hz or negedge rst_n) begin
        if (!rst_n) begin
            ...
            digit_sel <= digit_sel + 1;  // 00→01→10→11循环
        end
    end

    // 位选信号解码（AN3=最左→东西十位，AN2=东西个位，AN1=南北十位，AN0=最右→南北个位）
    always @(*) begin
        case(digit_sel)
            2'b00: {AN3, AN2, AN1, AN0} = 4'b0111;  // 选中东西十位（AN3）
            2'b01: {AN3, AN2, AN1, AN0} = 4'b1011;  // 选中东西个位（AN2）
            2'b10: {AN3, AN2, AN1, AN0} = 4'b1101;  // 选中南北十位（AN1）
            2'b11: {AN3, AN2, AN1, AN0} = 4'b1110;  // 选中南北个位（AN0）
            default: {AN3, AN2, AN1, AN0} = 4'b1111; // 默认全灭
        endcase
    end
    ...
    always @(*) begin
        case(current_digit)
            ...
            default: {CA, CB, CC, CD, CE, CF, CG} = 7'b1111111; // 全灭
        endcase
    end
