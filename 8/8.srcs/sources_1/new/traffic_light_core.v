...
// 时间参数（单位：秒）
// 正常模式
parameter NORMAL_GREEN_EW  = 20;    // 东西绿灯20秒
parameter NORMAL_YELLOW    = 5;     // 黄灯5秒
parameter NORMAL_RED_EW    = 15;    // 东西红灯15秒
parameter NORMAL_GREEN_NS  = 15;    // 南北绿灯15秒
parameter NORMAL_RED_NS    = 20;    // 南北红灯20秒

// 高峰模式
parameter PEAK_GREEN_EW    = 25;    // 东西绿灯25秒
parameter PEAK_YELLOW      = 3;     // 黄灯3秒
parameter PEAK_RED_EW      = 10;    // 东西红灯10秒
parameter PEAK_GREEN_NS    = 10;    // 南北绿灯10秒
parameter PEAK_RED_NS      = 25;    // 南北红灯25秒
...
// 内部信号（32位计数器覆盖25秒×100MHz=2.5e9周期）
...
always @(*) begin
    ...
    case (current_state)
        S_EW_GREEN: begin
            // 绿灯计时到（green_ew秒 × 100MHz时钟周期）
            if (cnt >= green_ew * 100_000_000) begin
                next_state = S_EW_YELLOW;
            end
        end
        S_EW_YELLOW: begin
            // 黄灯计时到（yellow秒 × 100MHz）
            if (cnt >= yellow * 100_000_000) begin
                next_state = S_EW_RED;
            end
        end
        S_EW_RED: begin
            // 红灯计时到（red_ew秒 × 100MHz）
            if (cnt >= red_ew * 100_000_000) begin
                next_state = S_EW_GREEN;
            end
        end
...
always @(posedge clk) begin
    case (current_state)
        S_EW_GREEN: begin
            ...
            count_ew <= green_ew - (cnt / 100_000_000);  // 秒级倒计时
            count_ns <= red_ns - (cnt / 100_000_000);
        end
        S_EW_YELLOW: begin
            light_ew <= 3'b010;  // 东西绿=0，黄=1，红=0
            light_ns <= 3'b010;  // 南北绿=0，黄=1，红=0
            count_ew <= yellow - (cnt / 100_000_000);
            count_ns <= yellow - (cnt / 100_000_000);
        end
        S_EW_RED: begin
            light_ew <= 3'b100;  // 东西绿=0，黄=0，红=1
            light_ns <= 3'b001;  // 南北绿=1，黄=0，红=0
            count_ew <= red_ew - (cnt / 100_000_000);
            count_ns <= green_ns - (cnt / 100_000_000);
        end
        default: begin
            light_ew <= 3'b001;
            light_ns <= 3'b100;
