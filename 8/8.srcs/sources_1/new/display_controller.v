...
module display_controller(
    ...
    output reg AN0, AN1, AN2, AN3  // 位选（AN=1选中，硬件左→右=AN3→AN2→AN1→AN0）
);
...
// 2. 段选编码生成（核心修正：匹配硬件左→右=AN3→AN2→AN1→AN0布局）
always @(*) begin
    case (mode)
        MODE_EMERG: seg_code = CODE_0;  // 应急：全0（0000）
        MODE_SLEEP: seg_code = CODE_9;  // 休眠：全9（9999）
        default: begin  // 左2=东西（AN3=十位，AN2=个位），右2=南北（AN1=十位，AN0=个位）
            case (scan_bit)
                2'b00: seg_code = count2code(count_ns % 10);  // AN0：右1→南北个位
                2'b01: seg_code = count2code(count_ns / 10);  // AN1：右2→南北十位
                2'b10: seg_code = count2code(count_ew % 10);  // AN2：左2→东西个位
                2'b11: seg_code = count2code(count_ew / 10);  // AN3：左1→东西十位
...
// 3. 位选信号生成（AN=1选中，逐位轮流，硬件左→右=AN3→AN2→AN1→AN0）
always @(*) begin
    AN0 = 1'b0;
    AN1 = 1'b0;
    AN2 = 1'b0;
