...
module display_controller_sim(
    ...
    input [5:0] count_ew,       // 东西方向倒计时（0~59，单位：ms）
    input [5:0] count_ns,       // 南北方向倒计时（0~59，单位：ms）
...
// 内部信号：扫描时钟（250Hz，对应每个数字亮4ms，刷新周期16ms，适配短周期仿真）
...
// 分频生成250Hz扫描时钟（刷新周期16ms，不随时间缩放改变，保证显示无闪烁）
always @(posedge clk_100m) begin
    if (div_cnt >= 18'd199_999) begin  // 100MHz ÷ (2×200000) = 250Hz
        div_cnt <= 18'd0;
        clk_scan <= ~clk_scan;
    end else begin
...
always @(*) begin
    case (mode)
        ...
        default: begin  // 正常/高峰模式：显示倒计时（单位ms，直接取十位/个位）
            case (digit_sel)
                2'b00: current_digit = count_ew / 10;  // AN3：东西十位（如20ms→2）
                2'b01: current_digit = count_ew % 10;  // AN2：东西个位（如20ms→0）
